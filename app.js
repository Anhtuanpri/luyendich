...
const store={get items(){try{return JSON.parse(localStorage.getItem(S.items)||"[]")}catch(_){return[]}},set items(v){localStorage.setItem(S.items,JSON.stringify(v||[]))},get idx(){return parseInt(localStorage.getItem(S.idx)||"0",10)||0},set idx(v){localStorage.setItem(S.idx,String(v|0))},get attempts(){try{return JSON.parse(localStorage.getItem(S.attempts)||"[]")}catch(_){return[]}},set attempts(v){localStorage.setItem(S.attempts,JSON.stringify(v||[]))},get key(){return localStorage.getItem(S.api)||""},set key(v){localStorage.setItem(S.api,v||"")},get mdl(){return localStorage.getItem(S.mdl)||"gpt-4o-mini"},set mdl(v){localStorage.setItem(S.mdl,v||"gpt-4o-mini")},get ep(){return localStorage.getItem(S.ep)||"https://api.openai.com/v1/chat/completions"},set ep(v){localStorage.setItem(S.ep,v||"https://api.openai.com/v1/chat/completions")},get dict(){try{return JSON.parse(localStorage.getItem(S.dict)||"[]")}catch(_){return[]}},set dict(v){localStorage.setItem(S.dict,JSON.stringify(v||[]))},get avatar(){return localStorage.getItem(S.avatar)||""},set avatar(v){localStorage.setItem(S.avatar,v||"")},get name(){return localStorage.getItem(S.name)||""},set name(v){localStorage.setItem(S.name,v||"")},get wrp(){return localStorage.getItem(S.wrp)||""},set wrp(v){localStorage.setItem(S.wrp,v||"")},get wri(){return localStorage.getItem(S.wri)||""},set wri(v){localStorage.setItem(S.wri,v||"")}};
const root=document.documentElement;const prefersDark=matchMedia("(prefers-color-scheme: dark)").matches;if(localStorage.getItem("theme")==="dark"||(!localStorage.getItem("theme")&&prefersDark)){root.classList.add("dark");$("#toggleTheme").textContent="🌙"}$("#toggleTheme").onclick=()=>{root.classList.toggle("dark");const d=root.classList.contains("dark");localStorage.setItem("theme",d?"dark":"light");$("#toggleTheme").textContent=d?"🌙":"🌤️"};
function applyFooterPadding(){const f=$("#footerNav");if(!f)return;const h=f.offsetHeight||78;root.style.setProperty("--footer-h",h+"px")}
new ResizeObserver(applyFooterPadding).observe(document.body);window.addEventListener("load",applyFooterPadding);window.addEventListener("resize",applyFooterPadding);applyFooterPadding();
...
let items=store.items.length?store.items:["Chúng tôi đang xem phim ở rạp chiếu phim, phim rất hay.","Họ đang xây một ngôi nhà mới, nó sẽ rất lớn.","Tôi đang học lái xe, tôi rất lo lắng."];
let idx=clamp(store.idx,0,items.length-1);
function renderPractice(){const total=items.length||1;idx=clamp(idx,0,total-1);$("#viBox").textContent=items[idx]||"—";$("#answer").value="";$("#gIdea").textContent="—";$("#gVocab").textContent="—";$("#gGrammar").textContent="—";$("#gradeHeader").textContent="Chưa chấm.";$("#gradeDiff").innerHTML="";$("#gradeDiff").className="diff mono scrollBox simple";$("#gradeBreakdown").innerHTML="";$("#counter").textContent=`Câu ${idx+1}/${total}`;$("#progbar").style.width=`${Math.round(((idx+1)/total)*100)}%`;store.items=items;store.idx=idx}
...
async function callOpenAI(sys,usr,max_tokens=900){
  ...
  const r=await fetch(url,{method:"POST",headers,body:JSON.stringify({model,temperature:0.2,max_tokens,messages:[{role:"system",content:sys},{role:"user",content:usr}]})});
...
function similarity(a,b){const tok=s=>new Set((s||"").toLowerCase().match(/[a-z']+/g)||[]);const A=tok(a),B=tok(b);let inter=0;A.forEach(x=>{if(B.has(x))inter++});const uni=A.size+B.size-inter||1;return inter/uni}
...
function metrics(text){const t=(text||"").replace(/\s+/g," ").trim();const words=(t.match(/[A-Za-z]+(?:'[A-Za-z]+)?/g)||[]);const wordCount=words.length;const types=new Set(words.map(w=>w.toLowerCase())).size;const ttr=wordCount?types/wordCount:0;const sentences=(t.split(/[.!?]+/).map(s=>s.trim()).filter(Boolean));const sentCount=sentences.length||0;const avgLen=wordCount&&sentCount?wordCount/sentCount:wordCount;const clauseMarkers=(t.match(/\b(and|but|because|although|however|which|that|who|while|whereas|moreover|therefore|in addition|on the other hand)\b/gi)||[]).length;const linking=(t.match(/\b(firstly|secondly|finally|overall|in general|to conclude|in conclusion|as a result|for example|for instance)\b/gi)||[]).length;return{wordCount,ttr,sentCount,avgLen,clauseMarkers,linking}}
function capByMode(text,mode){const m=metrics(text);let cap=9;if(m.wordCount<10)cap=4.5;else if(m.sentCount<=1&&m.wordCount<20)cap=Math.min(cap,5);if(m.clauseMarkers<1)cap=Math.min(cap,6);if(m.avgLen<10)cap=Math.min(cap,6);if(m.ttr<0.35)cap=Math.min(cap,6.5);if(m.ttr<0.28)cap=Math.min(cap,6);if(mode==="task2"){if(m.wordCount<180)cap=Math.min(cap,6);else if(m.wordCount<220)cap=Math.min(cap,7)}if(mode==="task1"){if(m.wordCount<150)cap=Math.min(cap,6.5);if(!/\boverall|in general\b/i.test(text))cap=Math.min(cap,6.5)}if(mode==="spoken"){if(m.wordCount<120||m.sentCount<3)cap=Math.min(cap,6)}return{cap,m}}
...
function splitBySentences(text){const s=(text||"").trim().replace(/\s+/g," ");return s.length?s.split(/(?<=[.!?])\s+(?=[A-Z0-9])/).filter(Boolean):[]}
function groupChunks(sents,maxWords=60,maxSents=3){const groups=[];let cur=[];let wc=0;for(const s of sents){const w=(s.match(/[A-Za-z]+/g)||[]).length;if(cur.length&&(wc+w>maxWords||cur.length>=maxSents)){groups.push(cur.join(" "));cur=[s];wc=w}else{cur.push(s);wc+=w}}if(cur.length)groups.push(cur.join(" "));return groups}
...
$("#btnSuggest").onclick=async()=>{
  ...
  try{setBusy(true);$("#hint").textContent="Đang gợi ý...";const js=parseJSON(await callOpenAI(sys,"Vietnamese: "+vi,450));
    if(!js)throw"Lỗi";const showVI=$("#toggleViGloss").checked;
...
$("#btnTranslateNow").onclick=async()=>{
  ...
  try{setBusy(true);$("#hint").textContent="Đang dịch...";const en=await callOpenAI(sys,vi,250);$("#answer").value=en.trim();$("#hint").textContent="Đã dịch."}catch(e){$("#hint").textContent="Lỗi: "+e}finally{setBusy(false)}
}
$("#btnGradeSimple").onclick=async()=>{
  const vi=items[idx],en=($("#answer").value||"").trim();
  if(!en){$("#hint").textContent="Bạn chưa nhập bản dịch.";return}
  try{setBusy(true);$("#hint").textContent="Đang chấm...";const r=await gradeSimpleSmart(vi,en);
...
$("#btnGradeIELTS").onclick=async()=>{
  const vi=items[idx],en=($("#answer").value||"").trim();
  if(!en){$("#hint").textContent="Bạn chưa nhập bản dịch.";return}
  try{setBusy(true);$("#hint").textContent="Đang chấm IELTS...";const r=parseJSON(await callOpenAI(IELTS_PROMPT,`VI: ${vi}\nEN: ${en}`,900));
    if(!r||!r.criteria)throw"Lỗi";showGradingResult(en,r,$("#gradeHeader"),$("#gradeDiff"),$("#gradeBreakdown"),"IELTS Overall","spoken");
...
$("#btnGSGen").onclick=async()=>{
  ...
  try{setBusy(true);$("#gsHint").textContent="Đang tạo...";const js=parseJSON(await callOpenAI(sys,usr,600));
    if(!js?.sentences?.length)throw"Lỗi";$("#gsOut").textContent=js.sentences.join("\n");$("#gsHint").textContent="Đã tạo."}catch(e){$("#gsOut").textContent="Lỗi: "+e;$("#gsHint").textContent=""}finally{setBusy(false)}
};
...
$("#btnSpGen").onclick=async()=>{
  ...
  try{setBusy(true);$("#spHint").textContent="Đang tạo...";const js=parseJSON(await callOpenAI(sys,usr,800));
    if(!js?.spoken_vi)throw"Lỗi";$("#spOut").textContent=js.spoken_vi.trim();$("#spHint").textContent="OK"}catch(e){$("#spOut").textContent="Lỗi: "+e;$("#spHint").textContent=""}finally{setBusy(false)}
};
$("#btnSpUse").onclick=()=>{const t=$("#spOut").textContent.trim();if(!t){$("#spHint").textContent="Chưa có";return}items=items.concat([t]);store.items=items;idx=items.length-1;$("#spHint").textContent="Đã thêm";setTimeout(()=>{$("#spHint").textContent="";$("#tabPractice").click();renderPractice()},800)};
$("#btnSpTransGrade").onclick=async()=>{
  const vi=$("#spOut").textContent.trim();if(!vi){$("#spHint").textContent="Chưa có";return}
  try{setBusy(true);$("#spHint").textContent="Đang dịch...";const en=await callOpenAI("Translate VI to natural English essay",vi,1200);
    const r=parseJSON(await callOpenAI(IELTS_WR_SYS,`PROMPT:\n[SPOKEN]\n\nESSAY:\n${en}`,1200));
    if(!r||!r.criteria)throw"Lỗi";$("#tabWriting").click();$("#wrEssay").value=en.trim();
    showGradingResult(en,r,$("#wrHead"),$("#wrDiff"),$("#wrBreakdown"),"IELTS Overall (Spoken→EN)","spoken");
    $("#wrResult").style.display="grid";$("#spHint").textContent=""}catch(e){$("#spHint").textContent="Lỗi: "+e}finally{setBusy(false)}
};
$("#btnGenEssayVI").onclick=async()=>{
  ...
  try{setBusy(true);$("#ieltsGenHint").textContent="Đang tạo...";const js=parseJSON(await callOpenAI(sys,"",900));
    if(!js?.essay_vi)throw"Lỗi";$("#ieltsGenOut").textContent=js.essay_vi.trim();$("#ieltsGenHint").textContent="OK"}catch(e){$("#ieltsGenOut").textContent="Lỗi: "+e;$("#ieltsGenHint").textContent=""}finally{setBusy(false)}
};
$("#btnIELTSUse").onclick=()=>{const t=$("#ieltsGenOut").textContent.trim();if(!t){$("#ieltsGenHint").textContent="Chưa có";return}items=items.concat([t]);store.items=items;idx=items.length-1;$("#ieltsGenHint").textContent="Đã thêm";setTimeout(()=>{$("#ieltsGenHint").textContent="";$("#tabPractice").click();renderPractice()},800)};
$("#btnIELTSTransGrade").onclick=async()=>{
  ...
  try{setBusy(true);$("#ieltsGenHint").textContent="Đang dịch...";const sysTr=kind==="task1"?"Translate VI to IELTS Task1 English":"Translate VI to IELTS Task2 English";
    ...
    $("#wrResult").style.display="grid";$("#ieltsGenHint").textContent=""}catch(e){$("#ieltsGenHint").textContent="Lỗi: "+e}finally{setBusy(false)}
};
...
$("#btnGenPrompt").onclick=async()=>{
  ...
  try{setBusy(true);$("#wrPromptHint").textContent="Đang tạo...";const js=parseJSON(await callOpenAI(sys,usr,450));
    if(!js||!js.prompt)throw"Lỗi";$("#wrPrompt").value=js.prompt+(Array.isArray(js.hints)?("\n\nGợi ý:\n- "+js.hints.join("\n- ")):"");store.wrp=$("#wrPrompt").value;$("#wrPromptHint").textContent="OK"}catch(e){$("#wrPromptHint").textContent="Lỗi: "+e}finally{setBusy(false)}
};
$("#btnGradeEssay").onclick=async()=>{
  ...
  try{setBusy(true);$("#wrHint").textContent="Đang chấm...";const js=parseJSON(await callOpenAI(IELTS_WR_SYS,`PROMPT:\n${prompt||"(no prompt)"}\n\nESSAY:\n${essay}`,1200));
    if(!js||!js.criteria)throw"Lỗi";showGradingResult(essay,js,$("#wrHead"),$("#wrDiff"),$("#wrBreakdown"),"IELTS Overall",mode);
    $("#wrResult").style.display="grid";$("#wrHint").textContent="";addHistory({type:"writing",input:(prompt||"—").slice(0,80),score:js.overall_score,band:bandFromScore(js.overall_score),ok:false})
...
$("#btnTranslateTask").onclick=async()=>{
  const vi=$("#trTaskInput").value.trim();if(!vi){$("#trTaskHint").textContent="Chưa có";return}
...
$("#btnGradeTranslated").onclick=async()=>{
  const en=$("#trTaskOut").textContent.trim();if(!en){$("#trTaskHint").textContent="Chưa có";return}
  const mode=$("#trTaskType").value==="task1"?"task1":"task2";
  try{setBusy(true);$("#trTaskHint").textContent="Đang chấm...";const js=parseJSON(await callOpenAI(IELTS_WR_SYS,`PROMPT:\n[${mode.toUpperCase()}]\n\nESSAY:\n${en}`,1200));
    if(!js||!js.criteria)throw"Lỗi";$("#tabWriting").click();showGradingResult(en,js,$("#wrHead"),$("#wrDiff"),$("#wrBreakdown"),"IELTS Overall (Dịch)",mode);$("#wrResult").style.display="grid";$("#trTaskHint").textContent=""}catch(e){$("#trTaskHint").textContent="Lỗi: "+e}finally{setBusy(false)}
};
function appendMsg(t,m=false,ty=false){const r=el("div","msg"+(m?" me":""));const b=el("div","bubble");b.innerHTML=ty?'<span class="type">&nbsp;</span>':esc(t);r.appendChild(b);$("#chatBox").appendChild(r);$("#chatBox").scrollTop=$("#chatBox").scrollHeight;return b}
async function typeOut(e,t,s=12){e.innerHTML="";let i=0;return new Promise(a=>{const id=setInterval(()=>{e.textContent+=t[i++]||"";$("#chatBox").scrollTop=$("#chatBox").scrollHeight;if(i>=t.length){clearInterval(id);a()}},s)})}
let chatLock=false;
async function sendAsk(){if(chatLock)return;const q=($("#askInput").value||"").trim();if(!q){$("#askHint").textContent="Nhập câu hỏi.";return}chatLock=true;$("#askHint").textContent="";appendMsg(q,true);const b=appendMsg("",false,true);try{const sys="Bạn là trợ lý tiếng Anh hữu ích. Giải thích ngắn gọn, có ví dụ; nếu người dùng đưa câu, hãy sửa và nêu lý do. Dùng tiếng Việt khi giải thích, ví dụ bằng EN.";const ans=await callOpenAI(sys,q,650);await typeOut(b,ans.replace(/^```[\s\S]*?```/g,""),10)}catch(e){b.textContent="Lỗi: "+e}finally{chatLock=false}}
$("#btnAsk").onclick=sendAsk;
const askI=$("#askInput");if(askI&&!askI.dataset.enterBound){askI.addEventListener("keydown",e=>{if(e.key==="Enter"&&!e.shiftKey&&!e.isComposing&&!e.repeat){e.preventDefault();sendAsk()}});askI.dataset.enterBound="1"}

function renderHistDict(){const h=store.dict.slice(-14).reverse();$("#dictHistory").innerHTML=h.length?h.map(w=>`<span class="errtag">${esc(w)}</span>`).join(""):"Chưa có."}
renderHistDict();
async function dictAPI(q){const sys=`Bạn là từ điển EN⇄VI chuẩn. Trả JSON:
...
], "phrases":[{"phrasal":"","gloss_vi":"","example_en":"","example_vi":""}]}`;const out=await callOpenAI(sys,"Lookup: "+q,900);return parseJSON(out)}
function showDict(d){const b=$("#dictOut");b.innerHTML="";if(!d){b.innerHTML='<div class="mono">Không có dữ liệu.</div>';return}b.append(el("div","",`<h2 class="sectionTitle" style="margin:0">${esc(d.headword)} <span class="subtle">(${(d.pos||[]).join(", ")})</span></h2>`));b.append(el("div","subtle",`IPA UK: /${esc(d.ipa_uk||"-")}/ • IPA US: /${esc(d.ipa_us||"-")}/`));(d.senses||[]).forEach((s,i)=>b.append(el("div","card row",`
...
`)));if(Array.isArray(d.phrases)&&d.phrases.length){b.append(el("div","card row",`
...
`))}}
$("#btnLookup").onclick=async()=>{const q=($("#q").value||"").trim();if(!q){$("#dictHint").textContent="Nhập từ/cụm cần tra.";return}try{setBusy(true);$("#dictHint").textContent="Đang tra...";const data=await dictAPI(q);showDict(data);const hist=store.dict;hist.push(q);store.dict=hist.slice(-50);renderHistDict();$("#dictHint").textContent=""}catch(e){$("#dictOut").innerHTML='<div class="mono">Lỗi: '+esc(e+"")+"</div>";$("#dictHint").textContent=""}finally{setBusy(false)}};
$("#btnLookupFromVI").onclick=async()=>{const vi=(items[idx]||"").trim();if(!vi){$("#dictHint").textContent="Chưa có câu để gợi ý.";return}try{setBusy(true);$("#dictHint").textContent="Đang gợi ý...";const sys='Từ câu tiếng Việt sau, gợi ý 1–3 từ/cụm TIẾNG ANH nên tra. Trả JSON {"candidates":["..."]}';const js=parseJSON(await callOpenAI(sys,vi,200));const cand=js?.candidates?.[0]||"";if(!cand){$("#dictHint").textContent="Không gợi ý được.";setBusy(false);return}$("#q").value=cand;$("#dictHint").textContent="Đang tra "+cand+"...";const data=await dictAPI(cand);showDict(data);const hist=store.dict;hist.push(cand);store.dict=hist.slice(-50);renderHistDict();$("#dictHint").textContent=""}catch(e){$("#dictOut").innerHTML='<div class="mono">Lỗi: '+esc(e+"")+"</div>";$("#dictHint").textContent=""}finally{setBusy(false)}};
